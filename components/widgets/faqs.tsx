...
export interface FaqsProps extends WidgetProps {
  title?: string;
  subtitle?: string;
  tagline?: string;
  items?: Item[];
...
function FaqAccordionList({ items = [], className }: { items?: Item[]; className?: string }) {
...
export function FAQs({ id, isDark = false, bg, scrollMarginClass, title = '', subtitle = '', tagline = '', items = [], classes = {} }: FaqsProps) {
  return (
    <WidgetWrapper
      id={id}
      isDark={isDark}
